...
export async function listRefreshHistory(
  ...
  const full = await supabase
    .from("refresh_history")
    .select(
      "id, refresh_type, status, started_at, completed_at, source_filename, rows_read, rows_matching, distinct_work_orders, rows_created, rows_updated, rows_archived, rows_failed, error_message"
    )
    .order("started_at", { ascending: false })
    .limit(safeLimit);
  ...
  if (
    ...
    const { data, error } = await supabase
      .from("refresh_history")
      .select(
        "id, refresh_type, status, started_at, completed_at, rows_read, rows_created, rows_updated, rows_archived, error_message"
      )
      .order("started_at", { ascending: false })
      .limit(safeLimit);
    if (error) throw error;
