...
/** Normalize material_category; unknown values fall back to Flats. */
export function normalizeMaterialCategory(category) {
  const value = String(category || "").trim();
  if (value === MATERIAL_CATEGORIES.SHAPES) return MATERIAL_CATEGORIES.SHAPES;
  if (value === MATERIAL_CATEGORIES.PURCHASED_PARTS) {
    return MATERIAL_CATEGORIES.PURCHASED_PARTS;
  }
  return MATERIAL_CATEGORIES.FLATS;
}

/**
 * Untouched default Purchased Parts rows (UI placeholders) must not be persisted.
 * Any entered field or non-default status counts as real data.
 */
export function isBlankPurchasedPartLine(line) {
  if (normalizeMaterialCategory(line?.material_category) !== MATERIAL_CATEGORIES.PURCHASED_PARTS) {
    return false;
  }
  const empty =
    !String(line.material_type || "").trim() &&
    !String(line.supplier || "").trim() &&
    !String(line.material_po || "").trim() &&
    !String(line.ead || "").trim();
  const status = line.status || "Not Ordered";
  return empty && status === "Not Ordered";
}
